...
class Read_Block_Template extends Ability_Definition {
	...
	protected function ability(): array {
		return array(
			'name' => 'zoltiq/read-block-template',
			'args' => array(
				'label'               => __( 'Read Block Template', 'zoltiq-agents' ),
				'description'         => __( 'Reads a single block template by slug from the database, theme, or plugin. When the slug exists in multiple locations, returns "multiple_locations" with the candidate list — pick one with source / theme_type / plugin_slug.', 'zoltiq-agents' ),
				...
				'input_schema'        => array(
					...
					'properties'           => array(
						'slug'        => array(
							'type'        => 'string',
							'description' => __( 'Template slug (bare, e.g. "single", "index", "404").', 'zoltiq-agents' ),
						),
						'source'      => array(
							'type'    => 'string',
	...
	public function execute( array $input = array() ): array {
		...
		if ( empty( $locations ) ) {
			return array(
				'success'   => false,
				'message'   => sprintf( __( 'No template with slug "%s" was found. Use template-create to add one.', 'zoltiq-agents' ), $slug ),
				'locations' => array(),
			);
		}
	...
	private function collect_warnings( array $locations, array $selected ): array {
		...
		foreach ( $locations as $loc ) {
			if ( ( $loc['source'] ?? '' ) === 'plugin' && false === ( $loc['plugin_active'] ?? true ) ) {
				$warnings[] = sprintf( __( 'Plugin "%s" is inactive — its template will not register until the plugin is activated.', 'zoltiq-agents' ), $loc['plugin'] ?? '' );
			}
		}
	...
	private function materialise( array $loc ) {
		...
		if ( 'db' === $src ) {
			...
			if ( ! $post ) {
				return new \WP_Error( 'db_post_missing', __( 'Template post not found in the database.', 'zoltiq-agents' ) );
			}
