...
final class Pattern_Detector {
	...
	public static function select( array $locations, string $source = '', string $theme_type = '', string $plugin_slug = '' ) {
		if ( empty( $locations ) ) {
			return new \WP_Error( 'not_found', __( 'No pattern with this slug was found in the database, the active theme, or any plugin. Use block-pattern-create to add one.', 'zoltiq-agents' ) );
		}
		...
		if ( empty( $candidates ) ) {
			$error = new \WP_Error( 'not_found_at_source', __( 'No pattern with this slug exists at the requested source.', 'zoltiq-agents' ) );
		...
		if ( count( $candidates ) > 1 ) {
			$error = new \WP_Error(
				'multiple_locations',
				__( 'This slug exists in more than one location. Specify "source" (and "theme_type" or "plugin_slug") to pick one.', 'zoltiq-agents' )
			);
