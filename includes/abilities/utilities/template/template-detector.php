...
final class Template_Detector {
	...
	public static function select( array $locations, string $source = '', string $theme_type = '', string $plugin_slug = '' ) {
		if ( empty( $locations ) ) {
			return new \WP_Error( 'not_found', __( 'No template with this slug was found in the database, the active theme, or any plugin. Use template-create to add one.', 'zoltiq-agents' ) );
		}
		...
		if ( empty( $candidates ) ) {
			$error = new \WP_Error( 'not_found_at_source', __( 'No template with this slug exists at the requested source.', 'zoltiq-agents' ) );
